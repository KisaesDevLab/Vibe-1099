...
The app version (`0.1.20`) is single-sourced across `package.json`, `appliance/manifest.yaml`
...
```bash
git tag v0.1.20 && git push origin v0.1.20      # tag push
# — or — Actions → Release → Run workflow → version = 0.1.20   (also creates the vX.Y.Z tag)
```

The appliance can either **build locally** (default) or **pull the published images**. Compose
image refs are `${VIBE1099_REGISTRY:-}vibe1099-<svc>:${VIBE1099_VERSION:-0.1.20}`, so:

- `VIBE1099_REGISTRY=` (empty, default) → builds `vibe1099-app:0.1.20` from source.
- `VIBE1099_REGISTRY=ghcr.io/kisaesdevlab/` + `VIBE1099_VERSION=0.1.20` → `docker compose pull &&
